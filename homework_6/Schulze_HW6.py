...
ax.set(title = '1 to 1 plot of actual versus predicted flow', xlabel='flow t-1', ylabel='flow t', yscale='log', xscale='log')
ax.plot(np.sort(train['flow_tm1']), np.sort(q_pred_train), label='AR model', color = 'black')
ax.legend()
fig.set_size_inches(5, 5)  # Set to a square window to focus on the 1:1 nature.
